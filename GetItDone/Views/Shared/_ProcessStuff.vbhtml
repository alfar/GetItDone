...
<table class="workflow" border="0">
        ...
        @For Each ctx As GetItDone.ContextListModel In Model.Contexts
            @<li>@Html.ActionLink(ctx.Name, "AssignContext", "Task", New With {.Id = Model.Id, .Context = ctx.Id, .From = ViewBag.From}, Nothing)</li>
        Next
            <li>@Using Html.BeginForm("AssignNewContext", "Task", New With {.Id = Model.Id, .From = ViewBag.From})
    ...
    <tr><td rowspan="2"></td><td class="goal">@Html.ActionLink("Some day/Maybe", "FutureProject", "Task", New With {.Id = Model.Id, .From = ViewBag.From}, Nothing)</td><td></td><td class="goal">Reference</td><td rowspan="2"></td><td class="goal">@Html.ActionLink("Do it", "Finish", "Task", New With {.Id = Model.Id, .From = ViewBag.From}, Nothing)</td><td rowspan="2"></td><td class="goal">@Html.ActionLink("Delegate", "Assign", "Task", New With {.Id = Model.Id, .From = ViewBag.From}, Nothing)</td><td rowspan="2" colspan="4"></td><td rowspan="2"></td></tr>
