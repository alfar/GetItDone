...
<div id="thebigquestion">What is '@Model.Title'? <a class="command" href="@Url.Action("Edit", "Task", New With {.id = Model.Id, .From = ViewBag.From})"><img alt="Edit" src="@Url.Content("~/Content/themes/base/images/edit.png")" /></a></div>
<table class="workflow" border="0">
    ...
    <tr><td rowspan="2"></td><td class="goal">@Html.ActionLink("Some day/Maybe", "FutureProject", New With {.Id = Model.Id, .From = ViewBag.From})</td><td></td><td class="goal">Reference</td><td rowspan="2"></td><td class="goal">@Html.ActionLink("Do it", "Finish", "Task", New With {.Id = Model.Id, .From = ViewBag.From}, Nothing)</td><td rowspan="2"></td><td class="goal">@Html.ActionLink("Delegate", "Assign", New With {.Id = Model.Id, .From = ViewBag.From})</td><td rowspan="2" colspan="4"></td><td rowspan="2"></td></tr>
